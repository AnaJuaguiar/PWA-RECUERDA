  ...
  if (lists && lists.length) {
    ...
    emptyListCtn.innerHTML = `
      <div class="flex flex-col items-center">
        <img class="w-28 mb-4" src="assets/AddFirst.png" alt="Nenhuma Nota"/>
        <p>Nenhuma lista criada.</p>
        <p>Clique no + para adicionar uma nota.</p>
      </div>
    `;
...
function moveToAddListScreen() {
  ...
  createCtn.innerHTML = `
        ...
        <input id='listName' class="border rounded-lg p-1 pl-2" type="date" placeholder="Data do dia..."/>
        <div id="divider" class="py-4">
          <div class="border-t border-gray-300"></div>
        </div>
        <textarea rows="4" "id='noteDescription' class="border rounded-lg p-1 pl-2 mb-2 break-all	whitespace-pre-wrap  "  type="text" placeholder="Por qual motivo você é grato hoje?"></textarea>
        <div class="flex justify-center">
        <button class="border rounded-lg p-2 bg-amber-500 " type='submit'>Salvar Nota</button>
        </div>
        </form>
    `;
...
function moveToEditListScreen() {
  ...
  editCtn.innerHTML = `
        <div class='flex justify-center mb-2'>
          <button id='delete-list' class="border rounded-lg p-2 bg-rose-700 text-white">Excluir Nota</button>
        </div>
